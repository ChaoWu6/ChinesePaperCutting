...
from .fre import Fused_Fourier_Conv_Mixer
...
class StripAttentionBlock(nn.Module):
  def __init__(self, d_model, input_resolution, nhead=8, strip_width=7,
    ...
    self.attn4 = Fused_Fourier_Conv_Mixer(dim=d_model)
  ...
  def forward(self, x):
    ...
    b,n,c=x.size()
    h = w = int(math.sqrt(n))
    features_reshaped = x.permute(0, 2, 1).reshape(b, c, h, w)
    x4 = self.attn4(features_reshaped)
    ...
    k_x = torch.stack([x, x1, x2, x3,x4], dim=2)
